...
class Subprocess_calls_PDrive:
    ...
    def mount_Pdrive_sable(self, jnk_user, jnk_pass, logging):
        try:
            subprocess.check_call('sudo mount -t cifs %s %s -o user=%s,domain=%s,password=%s'%(self.p_Drive_path, self.p_Drive_mount_sable, jnk_user, self.domain, jnk_pass), shell=True)
